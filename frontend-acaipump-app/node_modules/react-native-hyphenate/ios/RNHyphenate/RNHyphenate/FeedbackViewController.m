...
#define NavigationHeight        [[UIApplication sharedApplication] statusBarFrame].size.height+44
...
@property (nonatomic) BOOL hasSendingImage;
@end
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:@"dismissViewController" object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        [[UIApplication sharedApplication] setStatusBarStyle:_style animated:NO];
        if (self.navigationController.presentedViewController) {
            [self.navigationController.presentedViewController dismissViewControllerAnimated:NO completion:^{
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if ([[SaveModel shared] getStatusBarLight]) {
    ...
    self.navigationController.navigationBar.tintColor = [AppTool colorWithKey:@"title"];
    [self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:[AppTool colorWithKey:@"title"]}];
    self.navigationController.navigationBar.barTintColor = [AppTool colorWithKey:@"navigationBar"];
    ...
    if (self.navigationController.viewControllers.count == 1) {
        ...
        [backButton setImage:[AppTool getImageWithName:@"barBack"] forState:UIControlStateNormal];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [self reloadDataAndScroll:_shouldScrollTableView];
}
...
- (void)createTableView{
    CGFloat height = ScreenHeight-[[UIApplication sharedApplication] statusBarFrame].size.height-44-BottomViewHeight;
    if ([self isIphoneX]) {
        height -= 34;
    }
    
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, NavigationHeight, ScreenWidth, height) style:UITableViewStylePlain];
...
- (void)createBottomView{
    //注意屏幕尺寸
    CGFloat yy = ScreenHeight-BottomViewHeight;
    if ([self isIphoneX]) {
        yy -= 34;
    }
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, yy, ScreenWidth, BottomViewHeight)];
...
//    [_textField setValue:[UIColor colorWithCSS:@"#878787"] forKeyPath:@"_placeholderLabel.textColor"];
    _textField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[AppTool FBlocalizedString:@"placeholder" AndComment:nil] attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14],NSForegroundColorAttributeName:[UIColor colorWithCSS:@"#878787"]}];
    [_bottomView addSubview:_textField];
    
    if ([[[SaveModel shared] getTextWord] length] > 1) {
        _textField.text = [[SaveModel shared] getTextWord];
        [[SaveModel shared] setTextWord:@""];
    }
    
    UIButton * imageSender = [UIButton buttonWithType:UIButtonTypeCustom];
    imageSender.frame = CGRectMake(_bottomView.bounds.size.width-50, (BottomViewHeight-40)/2, 40, 40);
    imageSender.imageView.contentMode = UIViewContentModeScaleAspectFit;
    [imageSender setImage:[AppTool getImageWithName:@"camera"] forState:UIControlStateNormal];
...
- (void) updateKeyboardStatus: (NSNotification * )notify show: (BOOL)isShow {
    ...
    [UIView animateWithDuration:animationDuration
                     animations:^{
                         if ([self isIphoneX]) {
                             if (y == 0) {
                                 _bottomView.frame = CGRectMake(0, ScreenHeight-BottomViewHeight-34, ScreenWidth, BottomViewHeight);
                             }else {
                                 _bottomView.frame = CGRectMake(0, ScreenHeight-y-BottomViewHeight, ScreenWidth, BottomViewHeight);
                             }
                             if (_tableView.contentSize.height < _tableView.frame.size.height) {
                                 CGFloat h = ScreenHeight-[[UIApplication sharedApplication] statusBarFrame].size.height-44-BottomViewHeight-y;
                                 if (y == 0) {
                                     h -= 34;
                                 }
                                 _tableView.frame = CGRectMake(0, NavigationHeight, ScreenWidth, h);
                             }else {
                                 CGFloat h = ScreenHeight-[[UIApplication sharedApplication] statusBarFrame].size.height-44-BottomViewHeight;
                                 if (y == 0) {
                                     h -= 34;
                                 }
                                 _tableView.frame = CGRectMake(0, NavigationHeight-y, ScreenWidth, h);
                             }
                         }else {
                             _bottomView.frame = CGRectMake(0, ScreenHeight-y-BottomViewHeight+1, ScreenWidth, BottomViewHeight);
                             CGFloat baseHeight = ScreenHeight-[[UIApplication sharedApplication] statusBarFrame].size.height-44-BottomViewHeight;
                             if (_tableView.contentSize.height < _tableView.frame.size.height) {
                                 _tableView.frame = CGRectMake(0, NavigationHeight, ScreenWidth, baseHeight-y);
                             }else {
                                 _tableView.frame = CGRectMake(0, NavigationHeight-y, ScreenWidth, baseHeight);
                             }
                         }
                     ...
                     completion:^(BOOL finished) {
                         self.shouldScrollTableView = YES;
...
- (void)presentPhotoLibrary:(UIButton *)button {
    ...
    UIActionSheet * sheet = [[UIActionSheet alloc] initWithTitle:nil
                                          ...
                                          destructiveButtonTitle:nil
                                               otherButtonTitles:[AppTool FBlocalizedString:@"canmerPhoto" AndComment:nil], [AppTool FBlocalizedString:@"choosePhoto" AndComment:nil], [AppTool FBlocalizedString:@"sendLog" AndComment:nil], nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (0 == buttonIndex) {
        ...
        [self sendTextMessage:[AppTool FBlocalizedString:@"didSendLog" AndComment:nil]];
        [NSNotificationCenter.defaultCenter postNotificationName:SendLog object:nil];
    }
}
...
- (void)showPhotoLibrary {
    @try {
        ...
        if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[AppTool FBlocalizedString:@"banner" AndComment:nil]
                                                            message:[AppTool FBlocalizedString:@"UnAuthorizedTip" AndComment:nil]
                                                           delegate:self
                                                  cancelButtonTitle:[AppTool FBlocalizedString:@"enter" AndComment:nil]
                                                  otherButtonTitles:nil];
            [alert show];
...
#pragma mark - 点击cell的URL代理方法
- (void)openURL:(FeedbackCell *)cell url:(NSURL *)url {
    if (@available(iOS 9.0, *)) {
        MySafari * vc = [[MySafari alloc] initWithURL:url];
...
+ (void)showNotificationWithMessage:(EMMessage *)message {
    ...
    if (options.displayStyle == EMPushDisplayStyleMessageSummary) {
        ...
        switch (messageBody.type) {
            ...
            {
                messageStr = [AppTool FBlocalizedString:@"imageMessage" AndComment:nil];
            }
                break;
            case EMMessageBodyTypeLocation:
            {
                messageStr = [AppTool FBlocalizedString:@"locationMessage" AndComment:nil];
            }
                break;
            case EMMessageBodyTypeVoice:
            {
                messageStr = [AppTool FBlocalizedString:@"voiceMessage" AndComment:nil];
            }
                break;
            case EMMessageBodyTypeVideo:{
                messageStr = [AppTool FBlocalizedString:@"videoMessage" AndComment:nil];
            }
                break;
            default:
                break;
